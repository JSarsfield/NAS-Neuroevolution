...
class EvolutionProc(object):
    ...
    def parallel_reproduce_eval(self, parent, n_net_inputs, n_net_outputs, env, gym_env_string):
        #parent = pickle.loads(parent)
        # Reproduce from parent genomes
        if type(parent[-1]) is not bool:  # Two parent genomes so crossover
            self.genome, self.new_structures = self.crossover(parent[0], parent[1])
        else:  # One parent genome so mutate
            self.genome, self.new_structures = self.copy_with_mutation(parent[0], mutate=parent[1])
        # Create genome graph
        self.genome.create_graph()
        # Create net from genome
        net = Substrate().build_network_from_genome(self.genome, n_net_inputs, n_net_outputs)
        if not net.is_void:
        ...
        self.fitness = net.fitness
        self.genome.net = None
        print("returned")
        return (self.genome, self.fitness, self.new_structures)
        ...
        while i < len(g1.gene_links) or j < len(g2.gene_links):
            if i < len(g1.gene_links) and j < len(g2.gene_links):
                ...
                if j == len(g2.gene_links):
                    ...
                    links_to_add.append(g2.gene_links[j])
                    j += 1
        return self.create_new_genome(nodes_to_add, links_to_add, g1.cppn_inputs, sub_width, sub_height)

    def copy_with_mutation(self, g1, mutate=True):
        """ copy a genome with mutation """
        return self.create_new_genome(g1.gene_nodes_in + g1.gene_nodes,
                                 g1.gene_links,
                                 g1.cppn_inputs,
                                 g1.substrate_width,
                                 g1.substrate_height,
    ...
    def create_new_genome(self, nodes_to_add, links_to_add, cppn_inputs, sub_width, sub_height, mutate=True):
        ...
        if mutate:
            new_structures = self.mutate_structural(gene_nodes, gene_links)  # This is performed on master thread to ensure only new genes are added to gene pool
        else:
            new_structures = []
    ...
    def get_node_from_hist_marker(self, gene_nodes, hist_marker):
    ...
    def mutate_structural(self, gene_nodes, gene_links):
        ...
        if event(node_add_prob):
            ...
            in_node = self.get_node_from_hist_marker(gene_nodes, old_link[1])
            out_node = self.get_node_from_hist_marker(gene_nodes, old_link[2])
